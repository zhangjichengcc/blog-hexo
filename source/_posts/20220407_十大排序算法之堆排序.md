...
> 还是上面的例子，我们看一下完全二叉树每一层的 `length` 之间的关系:

``` bash
              0                   # length = 1
         /        \  
      1              2            # length = 2 = provLength + curLength
    /   \          /   \        
   3     4        5     6         # length = 4 = 2 * 2
 /  \   /  \    /  \   /  \
7    8 9    10 11  12 13   14     # length = 8 = 

# leftChildIdx = 2 * parentIdx + 1
# rightChildIdx = 2 * parentIdx + 2
# lastUnLeafNodeIdx = Math.ceil(len / 2) - 1

- 最后一个非叶子节点idx = Math.ceil(length / 2) - 1;
```
